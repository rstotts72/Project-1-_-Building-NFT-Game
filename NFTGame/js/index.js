const address = '0x479dCBfb892d18190BF7f2300c6C219A64057286';
const abi = [
	{
		"inputs": [
			{
...
$(function() {
	...
	$(document).ready(function () {
		...
		$('#row').hide();
		Token.getTokensOfUser('0xfb1d31a3C52E4695EB506b23Fe701d461c25489b', function (err, result) {
		...
		$("#go").click(function () {
			...
			var id = document.getElementById("pets").value;
			document.getElementById('petId').innerHTML = id;
				Token.Endurance.call(id, function (err, result) {

					document.getElementById('endurance').innerHTML = result;
					endurance = result;
				});

			Token.Damage.call(id, function (err, result) {

				document.getElementById('damage').innerHTML = result;
			});

			Token.Magic.call(id, function (err, result) {

				document.getElementById('magic').innerHTML = result;
			});

			Token.LastMeal.call(id, function (err, result) {
			...
			$("#feed").click(function () {
				Token.Feed.sendTransaction(id, function (err, hash) { })
			});
		});
        });
	...
	if (typeof (web3) === 'undefined') {
		...
		ethereum.enable();
			...
			console.log("Connect to Ropsten");
